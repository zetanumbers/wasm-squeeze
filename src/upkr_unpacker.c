...
int upkr_decode_bit(Context* cx, int context_index) {
    ...
    while(cx->upkr_state < 32768) {
        if(cx->upkr_bits_left == 0) {
            cx->upkr_current_byte = *cx->upkr_data_ptr++;
            cx->upkr_bits_left = 8;
        }
        cx->upkr_state = (cx->upkr_state << 1) + (cx->upkr_current_byte & 1);
        cx->upkr_current_byte >>= 1;
        --cx->upkr_bits_left;
    }
#else
